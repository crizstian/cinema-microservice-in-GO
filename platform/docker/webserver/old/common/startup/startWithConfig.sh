...
[ -e /tmp/envconsul.hcl.tmpl ] && rm -f /tmp/envconsul.hcl.tmpl
touch /tmp/envconsul.hcl.tmpl
...
if [ -n "$APP_UUID" ] || [ -n "$ENABLE_DR" ] || [ -n "$APP_NAME" ]; then
		...
		exec envconsul -consul=${NOMAD_IP_aero}:8500 -log-level=$log_level -config=/tmp/envconsul.hcl /var/tmp/startup/startProcess.sh $START_CMD
else
