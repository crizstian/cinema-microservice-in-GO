...
func LoadConfigFromEnv() (*Config, error) {
	...
	// DB_SERVERS, DB_NAME are required; DB_REPLICA is optional (empty = standalone mode)
	if servers == "" || dbName == "" {
		return nil, fmt.Errorf("missing required environment variables: DB_SERVERS, DB_NAME")
	}
...
func Connect(ctx context.Context, cfg *Config) (*MongoConnection, error) {
	...
	// Support: auth/no-auth and standalone/replica-set modes
	var uri string
	var replicaSetParam string
	if cfg.ReplicaSet != "" {
		replicaSetParam = "&replicaSet=" + cfg.ReplicaSet
	}

	if cfg.User != "" && cfg.Pass != "" {
		uri = fmt.Sprintf(
			"mongodb://%s:%s@%s/%s?authSource=%s%s&w=majority&maxPoolSize=%d&minPoolSize=%d",
			cfg.User,
			cfg.Pass,
			serversStr,
			cfg.Database,
		...
		uri = fmt.Sprintf(
			"mongodb://%s/%s?w=majority%s&maxPoolSize=%d&minPoolSize=%d",
			serversStr,
			cfg.Database,
			replicaSetParam,
			cfg.MaxPoolSize,
...
func MongoDB(c chan *MongoConnection) {
	...
	// Crear contexto con timeout de 60 segundos
